...
//Properties here
@property(strong, nonatomic) SeededNumberGenerator *sng;
...
-(id) init {
    ...
    self.sng = [[SeededNumberGenerator alloc]init];
    return self;
}

- (void)seedScrambler:(NSString *) seedValue{
    [self.sng seed:seedValue];
}
...
- (int) resolveOri:(int[])arr len:(int)len base:(int)base {
    ...
    for (int i=0; i<len; i++) {
        if (arr[i] == -1) {
            arr[i] - base<2?0:[self.sng getIntBetweenMin:0 andMax:base-1];
            lastUnknown = i;
        }
        sum += arr[i];
    }
...
-(int) resolvePerm:(int[]) arr len:(int)len cntU:(int)cntU parity:(int)parity {
    if (arr[0] == -3) {
        ...
        return parity == -1 ? [self.sng getIntBetweenMin:0 andMax:1] : parity;
    }
    ...
    for (int i=0; i<len; i++) {
        if (val[i] != -1) {
            int j = [self.sng getIntBetweenMin:0 andMax:idx];
            int temp = val[i];
            val[idx++] = val[j];
            val[j] = temp;
        }
...
- (NSString *)randomState:(int[])cp co:(int[])co ep:(int[])ep eo:(int[])eo {
    ...
    if (cntUE < 2) {	//ep != STATE_RANDOM
        ...
        if (cp[0] == -3) {
            ...
            do {
                cpVal = [self.sng getIntBetweenMin:0 andMax:40319];
        ...
        if (ep[0] == -2) {
            do {
                epVal = [self.sng getIntBetweenMin:0 andMax:479001599];
    ...
    int val = [self.sng getIntBetweenMin:0 andMax:2186];
    CubieCube *c = [[CubieCube allocWithZone:NULL] initCubie:cpVal twist:co[0] == -2 ? val : (co[0] == -3 ? 0 : [self resolveOri:co len:8 base:3]) eperm:epVal flip:eo[0] == -2 ? [self.sng getIntBetweenMin:0 andMax:2047] : (eo[0] == -3 ? 0 : [self resolveOri:eo len:12 base:2])];
...
-(NSString*)scramble: (int) type {
    ...
    switch (type) {
        ...
        case 10:
            sw = [self.sng getIntBetweenMin:0 andMax:3]; cube = [self randomL6e:sw]; break;
        case 11:
            sw = [self.sng getIntBetweenMin:0 andMax:3]; cube = [self randomL10p:sw]; break;
        default:
            cube = @"";
    }
    //NSLog(@"%@", cube);
